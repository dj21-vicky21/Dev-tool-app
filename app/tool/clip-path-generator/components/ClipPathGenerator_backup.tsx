...
export default function ClipPathGenerator() {
      ...
      const handleMouseMoveGlobal = (e: MouseEvent) => {
        ...
        if (isDragging && activePoint) {
          ...
          if (snapToGrid && !shiftPressed) {
            ...
            // First, check for circle snapping with higher priority
            // This should come before horizontal and vertical line checks
            // Check for circular guides with precise distance calculation - first priority
            if (showAdvancedGuides && activeGuides.circles.length > 0) {
              // Find the closest circle to snap to with high precision
              let closestCircle = null;
              let minCircleDist = Infinity;
              
              for (const circle of activeGuides.circles) {
                // Calculate distance from point to circle perimeter precisely
                const distanceToCenter = Math.hypot(newX - circle.cx, newY - circle.cy);
                const radiusDiff = Math.abs(distanceToCenter - circle.r);
                
                // Use a smaller threshold that prioritizes exact matches to circle perimeters
                if (radiusDiff < 2 && radiusDiff < minCircleDist) {
                  minCircleDist = radiusDiff;
                  closestCircle = circle;
                }
              }
              
              // Snap to the closest circle if found with higher precision
              if (closestCircle && minCircleDist < 2) {
                // Calculate angle to determine point on circle
                const angle = Math.atan2(newY - closestCircle.cy, newX - closestCircle.cx);
                
                // Position point exactly on the circle perimeter
                newX = closestCircle.cx + Math.cos(angle) * closestCircle.r;
                newY = closestCircle.cy + Math.sin(angle) * closestCircle.r;
                
                // ALWAYS clear all other guides - this ensures only circle is highlighted
                snappedHorizontalGuides.length = 0;
                snappedVerticalGuides.length = 0;
                snappedAngleGuides.length = 0;
                
                // Clear visible grid lines
                newVisibleLines.length = 0;
                
                // Reset circle guides and add only this one
                snappedCircleGuides.length = 0;
                snappedCircleGuides.push(closestCircle);
                
                // Update guides immediately to prevent any flicker with other guides
                setSnappedGuides({
                  horizontal: [],
                  vertical: [],
                ...
                // Update visible lines immediately
                setVisibleGridLines(newVisibleLines);
                
                // Update shape immediately - this prevents any other snapping
                const updatedPoint = {
                  ...activePoint,
                  x: newX,
                  y: newY
                ...
                return; // Exit early to prevent any further snapping
              }
            }
            ...
            if (originalPos) {
              ...
              // If we're close to the original position (within 5 units), snap to it exactly
              if (distToOriginal < 5) {
                newX = originalPos.x;
                newY = originalPos.y;
            ...
            // Check for circle and line intersections if not snapped to a circle directly
            if (!snappedToIntersection && snappedCircleGuides.length === 0 && showAdvancedGuides && activeGuides.circles.length > 0) {
            ...
            // If not snapped to any intersection or circle, check individual guides
            if (!snappedToIntersection && snappedCircleGuides.length === 0) {
              // Check for horizontal snap lines with precise distance calculation
              ...
              for (const line of activeGuides.horizontal) {
                const distance = Math.abs(newY - line);
                // Only snap when very close to the line (within 1.5 units)
                if (distance < 1.5 && distance < minHorizontalDist) {
              ...
              // Only apply horizontal snapping if we're very close to a line
              if (snappedToHorizontal && closestHorizontalLine !== null) {
                newY = closestHorizontalLine;
                snappedHorizontalGuides.push(closestHorizontalLine);
                
                // Add to visible grid lines for visual feedback with stronger appearance
                newVisibleLines.push({
                  id: generateId(),
                  type: 'horizontal',
                  position: closestHorizontalLine,
                  opacity: 1.0
                });
              }
              
              // Check for vertical snap lines with precise distance calculation
              ...
              for (const line of activeGuides.vertical) {
                const distance = Math.abs(newX - line);
                // Only snap when very close to the line (within 1.5 units)
                if (distance < 1.5 && distance < minVerticalDist) {
                  minVerticalDist = distance;
                  closestVerticalLine = line;
                  snappedToVertical = true;
                }
              }
            
              // Only apply vertical snapping if we're very close to a line
              if (snappedToVertical && closestVerticalLine !== null) {
                newX = closestVerticalLine;
                snappedVerticalGuides.push(closestVerticalLine);
                
                // Add to visible grid lines for visual feedback with stronger appearance
                newVisibleLines.push({
                  id: generateId(),
                  type: 'vertical',
                  position: closestVerticalLine,
                  opacity: 1.0
                });
              }
            
              // If still no snapping, try angle guides - with improved angle snapping
              if (snappedHorizontalGuides.length === 0 && snappedVerticalGuides.length === 0 && 
                  activeGuides.angles && activeGuides.angles.length > 0) {
                ...
                for (const angleGuide of activeGuides.angles) {
                  ...
                  if (distanceToLine < 3.0 && distanceToLine < minAngleDist) {
  ...
  // Update the deduplicateIntersectionPoints function to be stricter about which intersection points to show
  const deduplicateIntersectionPoints = (points: Array<{
    ...
    let filteredPoints = points.filter(point => {
      ...
      // Only show circle intersection points when a circle is snapped
      if (point.type.includes('circle') && !hasCircleSnap) {
        return false;
      }
      
      // Only show horizontal line intersections when a horizontal line is snapped
      if (point.type.includes('line') && hasHorizontalSnap && !hasVerticalSnap) {
        // For horizontal-only snapping, only show points on the snapped horizontal line
        return snappedGuides.horizontal.some(h => Math.abs(h - point.y) < 0.5);
      }
      
      // Only show vertical line intersections when a vertical line is snapped
      if (point.type.includes('line') && !hasHorizontalSnap && hasVerticalSnap) {
        // For vertical-only snapping, only show points on the snapped vertical line
        return snappedGuides.vertical.some(v => Math.abs(v - point.x) < 0.5);
      }
      
      // Show line intersections only when both horizontal and vertical are snapped
    ...
    // If we have circle snaps, prioritize circle-related intersections
    if (hasCircleSnap && filteredPoints.some(p => p.type.includes('circle'))) {
      filteredPoints = filteredPoints.filter(p => p.type.includes('circle'));
    }
  ...
  const snapToExistingPoints = (newX: number, newY: number, excludePointId: string): { 
    ...
    // Enhanced snap radius for existing points - make them easier to target
    const pointSnapRadius = 5; // Larger snap radius for points
                ...
                <div className="guides-container">
                  ...
                  {isDragging && (() => {
                    ...
                    activeGuides.circles.forEach(circle => {
                      const isCircleSnapped = snappedGuides.circles.some(c => 
                        Math.abs(c.cx - circle.cx) < 1 && 
                        Math.abs(c.cy - circle.cy) < 1 && 
                        Math.abs(c.r - circle.r) < 1
                      );
                      
                      // Circle-horizontal intersections
                      activeGuides.horizontal.forEach(hLine => {
                        const isHorizontalSnapped = snappedGuides.horizontal.includes(hLine);
                        const isIntersectionSnapped = isHorizontalSnapped && isCircleSnapped;
                        const hasSnappedGuide = isHorizontalSnapped || isCircleSnapped;
                      ...
                      // Circle-vertical intersections
                      activeGuides.vertical.forEach(vLine => {
                        const isVerticalSnapped = snappedGuides.vertical.includes(vLine);
                        const isIntersectionSnapped = isVerticalSnapped && isCircleSnapped;
                        const hasSnappedGuide = isVerticalSnapped || isCircleSnapped;
                    ...
                    return uniqueIntersectionPoints.map((point, index) => (
                      <div 
                        ...
                        className={`guide-intersection ${point.isSnapped ? 'guide-intersection-snapped' : ''} ${point.type === 'circle-circle' ? 'circle-circle-intersection' : ''}`} 
                        style={{ 
                          top: `${point.y}%`, 
                          left: `${point.x}%`
                        }} 
